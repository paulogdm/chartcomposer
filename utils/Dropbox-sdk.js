...
function getDataFromConsumer(res) {
  ...
  console.log("isWindowOrWorker()",isWindowOrWorker())
  return res.blob();
...
function downloadRequest(path, args, auth, host, accessToken, options) {
  ...
  return fetch(getBaseURL(host) + path, fetchOptions).then(res => getDataFromConsumer(res).then(data => [res, data])).then(([res, data]) => responseHandler(res, data));
}

function parseBodyToType$1(res) {
  const clone = res.clone();
  return new Promise(resolve => {
    res.json().then(data => resolve(data)).catch(() => clone.text().then(data => resolve(data)));
  }).then(data => [res, data]);
}
...
function uploadRequest(path, args, auth, host, accessToken, options) {
  ...
  return fetch(getBaseURL(host) + path, fetchOptions).then(res => parseBodyToType$1(res)).then(([res, data]) => {
...
function rpcRequest(path, body, auth, host, accessToken, options) {
  ...
  return fetch(getBaseURL(host) + path, fetchOptions).then(res => parseBodyToType$2(res)).then(([res, data]) => {
    // maintaining existing API for error codes not equal to 200 range
    if (!res.ok) {
      // eslint-disable-next-line no-throw-literal
      throw {
...
client_id=${clientId}&client_secret=${clientSecret}`;
    ...
    return fetch(path, fetchOptions).then(res => parseBodyToType(res)).then(([res, data]) => {
      // maintaining existing API for error codes not equal to 200 range
      if (!res.ok) {
        // eslint-disable-next-line no-throw-literal
        throw {
