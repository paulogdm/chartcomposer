...
function exportHtml(song) {
    ...
    for ( var i = 0; i < aProperties.length; i++ ) {
        ...
        if ( song[prop] ) {
            aResults.push("<div class=song" + prop + getCss(prop) + ">" + ( "title" === prop || "subtitle" === prop ? "" : prop + ": " ) + song[prop] + "</div>");
        }
    }
    aResults.push("</div>");
...
function exportHtmlPart(aParts, i) {
    ...
    aResults.push("<div class=song" + part.type + sStyle + getCss(part.type) + ">");
    ...
    if ( part.lines ) {
        for ( var i = 0; i < part.lines.length; i++ ) {
            ...
            line = ( "comment" === part.type || "choruscomment" === part.type ? "<div class=lyriccomment style='float: left; padding: 4px 8px; padding-bottom: 0.2em; background: #DDD; line-height: 1;'>" + line + "</div><div style='clear: both;'></div>" : line );
            aResults.push("<div class=lyricline style='line-height: 1.8;'>" + line + "</div>");
        }
    }
